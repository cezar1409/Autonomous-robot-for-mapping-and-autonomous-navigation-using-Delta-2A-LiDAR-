...
class LidarDelta2A:
    ...
    def __init__( self, port, baudrate, timeout = 1 ):
        ...
        self._data = np.zeros((DATALEN*SECTORS,3))  # angle, distance, intensity
    ...
    def extractFrame( self, data ):
        for idx, bt in enumerate(data):
            if bt == FRAME_HEAD:
                ...
                for i in range(n):
                    angle = i*k + zerostart
                    intensity = frame[13 + i*3]  # First byte is intensity/power
                    dist = ((frame[13 + i*3 + 1]<<8) + frame[13 + i*3 + 2]) * 0.25
                    self._data[self._sector*n + i] = [angle, dist, intensity]
